...
export default async function DocumentationPage(props: {
	...
	const docsLang = getDocsLanguage(params.locale);
	const page = docsSource.getPage(params.path, docsLang);
...
export async function generateStaticParams() {
	const appLocales = Object.keys(config.i18n.locales);
	return docsSource.getPages().flatMap((page) => {
		const docsLang = page.locale ?? config.i18n.defaultLocale;
		return appLocales
			.filter((locale) => getDocsLanguage(locale) === docsLang)
			.map((locale) => ({ path: page.slugs, locale }));
	});
}
